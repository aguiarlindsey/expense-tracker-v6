...
// Returns { rule, category, subcategory, tags } for the first matching enabled
...
export function applyRules(expense, rules) {
  ...
  for (const rule of sorted) {
    if (matches(rule, expense)) {
      return { rule, category: rule.setCategory, subcategory: rule.setSubcategory || '', tags: rule.setTags || [] }
    }
  }
  return null
}
