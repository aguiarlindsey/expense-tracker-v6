...
function expenseToDb(e, userId) {
  return {
    ...
    recurring_days:      e.recurringDays ? parseInt(e.recurringDays, 10) : null,
...
function incomeToDb(i, userId) {
  return {
    ...
    household_id:    i.householdId || null,
...
function debtToDb(d, userId) {
  return {
    ...
    last_marked_month: d.lastMarkedMonth || null,
  }
}
...
function debtFromDb(row) {
  return {
    ...
    emiDueDay:      row.emi_due_day || null,
    lastMarkedMonth: row.last_marked_month || null,
    createdAt:      row.created_at || '',
    _rowVersion:    row.row_version || 1,
  }
}
...
function ruleToDb(r, userId) {
  return {
    ...
    set_tags:        r.setTags || [],
    set_is_recurring: r.setIsRecurring || false,
    set_recurring_period: r.setIsRecurring ? (r.setRecurringPeriod || null) : null,
    set_recurring_days:   r.setIsRecurring && r.setRecurringPeriod === 'custom' ? (parseInt(r.setRecurringDays, 10) || null) : null,
...
function ruleFromDb(row) {
  return {
    ...
    setIsRecurring: row.set_is_recurring || false,
    setRecurringPeriod: row.set_recurring_period || 'monthly',
    setRecurringDays:   row.set_recurring_days || null,
