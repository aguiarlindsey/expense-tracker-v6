...
| 44 | 2026-05-22 | v7.25.0 Epic 6 — Planning Tab complete: Phase 6.1 Planning Tab Shell (budget input local-draft fix — onFocus init/onBlur save prevents Supabase realtime cursor resets; Budget vs Actual empty state hint; category budget hint; ?ptab= URL param); budget inputs locale-aware formatting (en-IN lakh/crore vs en-US, 2dp, fmtBudgetDisplay + focusedBudget state, type=text+inputMode=decimal); Per-Category Budgets title → "May 2026" human-readable; Phase 6.2 Budget Rollover (rollover_enabled JSONB column on budgets table — add_budget_rollover.sql; rolloverEnabled in useStorage initial state/load/realtime/saveBudgets/retry/factory-reset; prevMonthStr + spentByCatPrevMonth + rolloverAmounts useMemos; per-category ↻ toggle button with badge showing +₹X; effectiveBgt = base + rollover in BudgetBar); Phase 6.3 Goal Improvements (GoalRing animated SVG ring replaces flat bar; GOAL_MILESTONES 🥉🥈🥇🏆 earned badges in --color-brand; colour-coded countdown ≤7d amber/overdue red; vertical dot-and-line contribution timeline; incognito masking on amounts); Design quality 8.1–8.3 (--color-warning + --color-brand semantic tokens with dark-mode variants; ~30 hardcoded hex → CSS vars in JSX; font-variant-numeric tabular-nums on 6 amount classes; --text-xs 0.7→0.75rem, 0.65/0.68rem→0.72rem font floor fixes); Skills installed globally: ui-ux-pro-max + design-system (C:/Users/aguia/.claude/skills/); i18n added to ROADMAP Optional/Future | 8.0 |
| 45 | 2026-05-27 | v7.26.0 Phase 7.1 Expense Templates: template manager modal (name/desc/amount/currency/category/subcategory/paymentMethod/tags/notes), save from existing expense, one-tap apply from FAB secondary menu, localStorage et_v6_templates, template count badge; v7.27.0 Phase 7.2 Bulk Edit: multi-select mode toggle (checkbox per row, shift-range select, select-all), floating action bar (X selected · category / payment / date patch · delete), ConfirmDialog for bulk delete, disabled swipe in bulk mode; v7.28.0 Phase 7.3 Subscription Tracker: Subs tab (renamed from Recur), monthlyEquiv() helper for period normalisation, auto-classify subscriptions by subcategory + category, summary tiles (monthly total / active count / yearly / cancel-risk count), zombie cancel-risk banner (45+ days since last charge), subscription cards with period badge + due countdown + cancel-risk badge + price-creep badge, other recurring + income sections; v7.29.0 Phase 7.4 Spend Streak + Gamification: gamificationData useMemo (spendByDay map, no-spend days this month, consecutive no-spend streak, under-budget streak + best-ever), milestone toasts (no-spend 5/10/15/20/25d, under-budget 3/7/14/30d), Streak bento tile on Overview (🔥/✨/💡 icon, large streak number, 🥉🥈🥇🏆 badges earned/unearned, no-budget fallback hint); v7.30.0 Phase 7.5 Merchant Analytics: Merchants third sub-tab in Analytics (Insights \| Trends \| Merchants), merchantData extended (first/last dates, topCat, months map, avg), selectedMerchant drill-down panel (stats + 6-month bar chart), merchant cards (category icon, name, cat badge, inline bar, txn count + avg + last date, total + % of spend, 6-month mini sparkbars), ⌘K command palette entry | 11.0 |
| 46 | 2026-05-27 | v7.31.0 Phase 7.6 Cash Flow Forecast: cashFlowForecast useMemo (variable daily rate last 30d avg, recurring monthly equiv → daily, income daily rate from recurring + variable last 90d, net daily rate); 📅 Forecast 4th sub-tab in Analytics; 30/60/90-day projection tiles (projected spend + surplus/deficit badge); daily rate breakdown card (variable vs recurring mini-bars, total outflow, income, net daily, deficit/surplus banner); spend trajectory SVG (past 30d actual bars + next 30d projected ghosted bars + dashed level line, TODAY divider, X-axis labels); upcoming recurring charges list (next 90d from nextDueDate, urgency badges ≤7d/≤14d, category icon); ⌘K nav-forecast command | ~6.0 |
| 47 | 2026-05-31 | v7.32.0 Phase 7.7 PDF Monthly Report: generatePDF.js (jsPDF + jspdf-autotable); 3-page export (cover+summary+categories, expenses, income); WinAnsi-safe currency formatter makeFmt() + PDF_SYMBOLS map + en-IN/en-US locale; shared tbl(doc, headColor) helper (theme:plain, valign:middle, minCellHeight, row dividers, colour bar on category col); cover page (dark band BAND_H=42mm, app title left, name+email+month-pill right, generated date, compact spacing); summary metrics row (SPENT/INCOME/NET SAVINGS/TRANSACTIONS, vertical dividers); category table (col order: Category \| % of Total \| Txns \| Amount, colour bars, right-aligned amounts); expenses page (Date \| Description \| Category \| Payment \| Amount); income page (Date \| Description \| Source \| Amount); footer (monthLabel + page N of N); FileDown strip button + Settings export entry; Phase 7.8 Natural Language Search: parseNLQuery.js pure parser (amounts: over/under/between; dates: today/yesterday/this week/last week/this month/last month/this year/last year/in Month Year; payment aliases: gpay→UPI/QR, credit→Credit Card etc.; categories: match CATS keys); nlQuery useMemo in Tracker; filteredExp merges NL + explicit filter states; indigo NL chips strip below search bar; placeholder updated; Bug fixes this session: tbl() ReferenceError doc not in scope (now passed as first arg); amount col wrapping (27→39mm, redistributed from desc); header text centered; category Amount col reordered to end; Payment/Txns header wrapping (col widths + HP padding 5→3mm); body halign center default, per-col overrides; PDF cover name+email+monthLabel stacked right; footer month-only label; compact header band | ~6.0 |
| 48 | 2026-06-03 | v7.33.0 Phase 8.1 Mobile Layout Audit: viewport-fit=cover (env safe-area now active on notched iPhones); 11 inputs/selects bumped to 1rem min (iOS zoom fix); toast stack lifted above bnav calc(68px + safe-area + 0.75rem); tracker 768px padding includes env(safe-area-inset-bottom); 100dvh on auth/lock/onboarding; touch targets (item-btn/modal-close/goal-btn/color-dot/chips 32–44px min); -webkit-tap-highlight-color transparent globally; overscroll-behavior contain on scroll panels + none on app-shell; @media(hover:none) disables stuck hover states on Android; removed deprecated -webkit-overflow-scrolling:touch (3 locations); Phase 8.2 Email Share: api/email-share.js Vercel function (Supabase JWT auth, Gmail SMTP, PDF + image attachments, recipient = logged-in user's email); generatePDF returnBase64 option (doc.output datauristring); ReceiptScanner passes _receiptImageB64 back via onResult; receipt email checkbox in expense form (shown post-scan only, preference persisted to et_v6_email_receipt_pref); handleEmailPDF in Tracker (generates PDF base64, POSTs to API, success/fail toast); Mail icon button in month strip + Email to me in Settings; Phase 8.3 Performance Audit: motion package removed (-160 kB, was installed but never imported); jsPDF + autotable converted to dynamic import (defers 421 kB until first PDF click); html2canvas (202 kB) deferred as jsPDF transitive dep; manualChunks: vendor-react/vendor-supabase/vendor-lucide for cache stability; handleExportPDF made async; initial JS load 53% reduction (~1600 kB → 747 kB raw / 214 kB gzip); Phase 8.4 V7 Docs: CHANGELOG session 48, ROADMAP 100%, version v7.33.0 final | 7.0 |
...
| 56 | 2026-09-18 | v8.0.0 Epic A Phase 0: Personalize settings shell (asset_type/asset_id generic tagging on expenses, downloadIcsReminder .ics helper, Personalize launched as a modal not an inline settings section); Phase 1: Vehicles (table + CRUD + ExpenseForm fuel/maintenance picker + PersonalizeModal cards with mileage/cost-per-km/PUC countdown); Phase 2: Credit Cards (table + CRUD + ExpenseForm picker + billing-cycle spend/utilization/due-date cards) — *retroactively logged, commits existed but hours were never recorded* | ~4.5 |
| 57 | 2026-09-18 → 2026-09-19 | v8.0.0 Epic A Phase 3: Houses (owned/rented/leased-out ownership, rent/electricity/maintenance due-day reminders); Phase 4: Other Assets (open-ended item registry — free-text category with chip suggestions, one-time + recurring EMI reminders, ungated ExpenseForm picker); Personalize modal no-tab-preselected fix; new Analytics → Vehicles tab (per-vehicle spend/mileage/service history + per-part warranty tracking); KMs-driven calculation iterated across several rounds of user feedback (odometer-delta → trip-meter priority → odometer-span subtraction → year-boundary via purchase date; unreliable week/month/tracked-since-logging tiles removed once shown to disagree with real fill-up data); fixed a pre-existing ReferenceError on Overview → Advanced date-preset filters (`today` was never defined, only `todayStr`) | ~6.5 |
| **Total** | | | **~241.5 h** |

---

## [Sessions 56–57] — v8.0.0: Epic A Personalization (Vehicles, Credit Cards, Houses, Other Assets) + Vehicle Analytics
_2026-09-18 → 2026-09-19_ · post-v7.37.0

### Session 56 — Epic A Phases 0–2: Shared Infra, Vehicles, Credit Cards (2026-09-18)
*Retroactively logged — these commits existed before this changelog entry was written; hours are estimated from commit timestamps and scope.*
- **Phase 0 — shared infrastructure** (`f576003`) — generic `asset_type`/`asset_id` columns on `expenses` (one lightweight tag pair reused by every future asset type instead of a new FK column each time); `downloadIcsReminder()` generic `.ics` Blob-download helper; "🏠 Personalize" launcher added to Settings
- **Personalize moved from inline section to modal** (`f797d61`) — Settings is a scroll of lightweight toggles; Personalize needed room for forms + card grids per asset type, so it became its own modal instead of a growing inline block
- **Phase 1 — Vehicles** (`bfef5f1`) — `vehicles` table (car/bike/scooter, fuel type, reg number, PUC date) + full CRUD; `ExpenseForm` vehicle picker in Fuel/Vehicle Maintenance sections with auto fuel-type fill and unit-aware labels (km/L, km/kg, km/kWh); PersonalizeModal vehicle cards (avg mileage, cost/km, next-service countdown, PUC reminder download)
- **Phase 2 — Credit Cards** (`00d66e7`) — `credit_cards` table (bank, last4, credit limit, billing cycle day, due day) + full CRUD, deliberately *not* reusing `asset_type`/`asset_id` since a card can be tagged to an expense alongside a vehicle/house simultaneously; dedicated `card_id` column instead; PersonalizeModal card cards (billing-cycle spend, utilization %, due-date reminder download)

### Session 57 — Epic A Phases 3–4, Vehicle Analytics, and 3 bug fixes (2026-09-18 → 2026-09-19)
- **Phase 3 — Houses** (`9519979`) — `houses` table; ownership widened mid-build from a plain owned/rented split to **owned / rented / leased-out** (renting a place to live vs. owning a place you rent out are different real states worth tracking distinctly); independent rent, electricity, and maintenance due-day reminders, each with its own `.ics` download
- **Personalize default-tab fix** (`d94018f`) — the modal was defaulting to the Vehicles pill open on launch; now opens with no pill selected until the user picks one
- **Phase 4 — Other Assets** (`d2383df`) — the original plan called for a rigid "Phones" table; changed to an open-ended registry (laptop, boat, jewelry, aircraft, anything) per user request. Free-text `category` with suggestion chips that prefill but don't restrict input; one-time reminder (label + date, e.g. "Warranty expires") plus a separate recurring EMI reminder; `ExpenseForm` gets an *ungated* "Tag to an item" picker, unlike the vehicle/house pickers which are gated by category, since items are open-ended by nature
- **Analytics → Vehicles tab** (`91d151d` and 6 follow-up fixes) — new per-vehicle breakdown added to the Analytics sub-nav: total/fuel/maintenance spend, avg efficiency, cost/km, and full service history with per-part warranty countdown (`ExpenseForm`'s service-parts editor gained a "warranty until" date field, no migration needed since `service_parts` is already `jsonb`). The KMs-driven metric went through several corrections as real fill-up data surfaced problems with each approach:
  - `d0908bb` — first pass computed distance from odometer deltas between logged fill-ups, added week/month/year breakdown tiles
  - `36dcd50` — added a "current odometer" tile once it became clear "distance since tracking started" and "the car's actual total mileage" are different numbers
  - `c613126` — removed the week/month tiles entirely (bucketing by short windows is only accurate if you happen to refuel twice inside that exact window, which isn't reliable) and switched "this year" to an odometer-boundary subtraction instead of summing deltas
  - `faaa537` — flipped priority to use the Trip A/B reading (reset at every refuel to record exactly "km since last fill-up") over odometer-delta pairing, after real numbers showed a large undercount
  - `032e069` → `1539a71` — replaced the summed "tracked" distance with a straight `latestOdo − firstLoggedOdo` subtraction, then removed both the "KMs Driven (Tracked)" tile and the "Monthly Breakdown" list altogether once neither could be shown to be reliably accurate
  - `b038ada` — fixed "KMs This Year" defaulting its baseline to 0 for any vehicle with no odometer reading logged before Jan 1, which silently showed a car's *entire lifetime mileage* as "this year" for any vehicle (e.g. one bought in 2019) that had only just started being logged in the app; now checks the vehicle's purchase date and only defaults to 0 for a vehicle actually purchased this year
- **Fixed a pre-existing `ReferenceError`** (`55c02cb`) — Overview → Advanced date-preset chips (This month/Last 3m/Last 6m/This year) referenced an undefined `today`; the component only ever defined `todayStr`. Bug predates this session (traced to Session 50, 2026-06-05) and only threw at runtime when "Advanced" was clicked, which is why `vite build` never caught it
- **Docs** — `upgrade-to-v8.md` updated after every phase (both the working copy and the mirrored copy in `~/.claude/plans`); in-app About section bumped to v8.0.0 with new Personalize/Vehicle Analytics badges and rows
...
## [Sessions 49–50] — Post-Launch Fixes + Filter Presets
_2026-06-03 → 2026-06-05_ · post-v7.33.0

### Session 49 — Performance + Security (2026-06-03)
- **Slow biometric / post-unlock load fixed** (`1ab3dc9`)
  - `useStorage`: two sequential `Promise.all` merged into one — all 6 tables (expenses, income, budgets, goals, contributions, trips) fetched in a single round-trip; `setLoading(false)` now waits for goals/trips too, eliminating the second sequential Supabase fetch after unlock
  - `api/biometric-auth-options` + `api/biometric-verify`: accept GET for warmup pings — return 200 immediately, no DB work
  - `LockScreen`: fires parallel GET pings to both endpoints on mount, so Lambda cold-start completes while the user is still looking at the lock screen instead of after they tap unlock
- **auth-cookie SameSite Lax → Strict** (`ac98089`) — reduces CSRF surface; Lax allows cookies on top-level navigations from external sites, Strict does not — correct for an endpoint never navigated to directly
- **Mouse scroll wheel restored** (`767994c`) — removed `overscroll-behavior-y:none` from `.app-shell`; its `overflow-x:hidden` implicitly made it a scroll container, and `overscroll-behavior-y:none` on a scroll container swallows wheel events without chaining them to the document, silently blocking all mouse scroll

### Session 50 — Search + Date Presets (2026-06-05)
- **Search expanded** (`15c20ed`) — `matchesSearch` in `dataHelpers.js` previously covered description/category/subcategory/paymentMethod/notes/source/tags; now also matches:
  - currency code (`BHD`, `USD`) and currency full name via CM lookup (typing "BAH" finds Bahraini Dinar expenses)
  - `paymentDescription` (e.g. "HDFC 4521"), `diningApp` (Zomato/Swiggy), `splitWith` (person name on split expenses)
- **Date preset chips in advanced filter** (`fe483f7`) — 6 one-tap presets above the custom date range: This month / Last month / Last 3m / Last 6m / This year / Last year; tapping sets `expDateFrom` + `expDateTo` and highlights the active chip, tapping again clears; month picker or custom date inputs clear the active preset to avoid conflicts

---

## [Session 47] — PDF Monthly Report + Natural Language Search
_2026-05-31_ · v7.32.0

### Phase 7.7 — PDF Monthly Report (`src/utils/generatePDF.js`)
- **`generateMonthlyPDF()`** — jsPDF + jspdf-autotable; A4 portrait; exported via FileDown button in month strip and Settings export section
- **Currency formatter** — `makeFmt(baseCurrency)`: `PDF_SYMBOLS` map of WinAnsi-safe ASCII prefixes (₹→Rs., ₽→RUB, ₿→BTC etc.); `en-IN` locale for South Asian currencies (lakh/crore grouping), `en-US` for all others; always `minimumFractionDigits: 2`
- **Cover page** — dark band (`BAND_H=42mm`) + `2mm` indigo accent; left: title / Monthly Report / Generated date; right: user display_name (bold) / email / month pill (indigo rounded rect); compact spacing; footer: `monthLabel` + Page N of N
- **Summary metrics row** — SPENT / INCOME / NET SAVINGS / TRANSACTIONS with vertical dividers; colour-coded values (red / green / conditional / indigo)
- **Category breakdown** — columns: Category (colour bar left edge, 3.5mm) | % of Total | Txns | Amount (right-aligned); sorted by spend descending
- **Expenses page** — Date | Description | Category | Payment | Amount; sorted newest first
- **Income page** — Date | Description | Source | Amount; only rendered when income exists
- **Shared `tbl(doc, headColor)` helper** — `theme:plain`, `valign:middle`, `minCellHeight:11mm`, `cellPadding HP/BP`, centred headers, row dividers via `didDrawCell`
- **Bug fixes during polish** — `tbl()` referenced `doc` from outer scope (ReferenceError; fixed by passing doc as first arg); amount cols widened 27→39mm (decimal wrapping); `HP` padding 5→3mm (Payment/Txns header wrapping); body `halign:center` default with per-column overrides; category Amount col moved to last position; cover name/email/pill layout; footer prefix removed

### Phase 7.8 — Natural Language Search (`src/utils/parseNLQuery.js`)
- **`parseNLQuery(raw, { catNames, payMethods })`** — pure parser; returns `{ text, categories, payment, amtMin, amtMax, dateFrom, dateTo, tokens }`
- **Amount patterns** — `over/above/more than N` → amtMin; `under/below/less than N` → amtMax; `between N and M` → range
- **Date patterns** — `today`, `yesterday`, `this/last week`, `this/last month`, `this/last year`, `in May`, `in March 2025` → ISO dateFrom/dateTo
- **Payment aliases** — `gpay/phonepe/paytm/upi` → UPI/QR; `credit/credit card` → Credit Card; `debit` → Debit Card; `net banking/netbanking` → Net Banking; `bnpl` → Buy Now Pay Later; etc.
- **Category matching** — case-insensitive word-boundary match against all `CATS` keys
- **Integration** — `nlQuery` useMemo in Tracker; `filteredExp` merges NL results with explicit filter UI (UI wins when both set); NL categories union with chip-selected categories
- **UI** — indigo chip strip below search bar showing each parsed token; remainder text shown as italic chip; falls back to plain `matchesSearch` when nothing recognised; search placeholder updated
