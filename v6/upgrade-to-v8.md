...
- [x] Settings launcher ("💳 Debt Payoff Planner") opening its own `DebtPlannerModal`, cloned from `PersonalizeModal`'s modal shell/form/card-grid conventions but without pills (single feature, not four asset types): add/edit/delete debt cards, each showing payoff date, total interest, a sampled amortization `LineChart`, and the extra-payment input. A stalled (payment-too-low) debt shows a warning instead of a misleading payoff date
