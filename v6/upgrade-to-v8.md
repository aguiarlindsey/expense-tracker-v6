...
**Status** (updated 2026-09-19): Epic A is fully built and shipped (Phases 0-4 — see below). Epic C1 (Long-Term Forecast) and C2 (Debt Payoff Planner) are now built and shipped. Epic B and Epic C3 (Smart Rules) remain fully scoped but not started. Epic B's schema/RLS design has been pressure-tested by a Plan agent and spot-verified against the actual codebase (see verification notes at the top of Epic B). Epic C scoped directly against existing code (`cashFlowForecast`, EMI/loan fields, OCR `ocr_corrections` precedent) with the user picking the lean version of all three sub-features. Ready for implementation to begin whenever the user picks one.
