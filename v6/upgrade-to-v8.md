...
- [x] `supabase/add_debts.sql`: `debts` table (`id text primary key`, `user_id`, `name`, `principal`, `interest_rate` [annual %], `term_months`, `minimum_payment`, `start_date`, `notes`, `row_version`, `updated_at`, `created_at`) + RLS + realtime + `bump_row_version()` trigger — **not yet run against production, user needs to paste into Supabase SQL editor**
- [x] `useStorage.js`: `debtToDb`/`debtFromDb` mappers, `debts` state, initial load, realtime subscribe, `addDebt`/`editDebt`/`deleteDebt` (cloned from `addVehicle`/`editVehicle`/`deleteVehicle` with offline-queue + conflict resolution), `factoryReset` cleanup, return object
- [x] Pure JS amortization helper (`src/utils/debtHelpers.js`): `calcMonthlyPayment()` (standard EMI formula), `generateSchedule()` (month-by-month balance/interest/principal, detects a payment too low to ever cover interest via a `stalled` flag instead of looping forever), `totalInterest()`, `simulateExtraPayment()` (diffs baseline vs. baseline+extra schedules)
- [x] "Extra payment" simulator wired into each debt card — live months-saved/interest-saved as you type an extra amount
- [x] UI lives as a third Planning sub-tab ("💳 Debt", after Budgets/Goals) rather than a Settings modal — moved there per user request 2026-09-19, since it's ongoing financial planning, not a one-time personalization registry. `DebtPlannerSection` (cloned from `PersonalizeModal`'s form/card-grid conventions, no pills needed for a single feature): add/edit/delete debt cards, each showing payoff date, total interest, a sampled amortization `LineChart`, and the extra-payment input. A stalled (payment-too-low) debt shows a warning instead of a misleading payoff date. `ptab=debt` URL param and ⌘K command palette entry included, matching the Budgets/Goals precedent
- [x] "📋 Full schedule" button per debt card opens `DebtScheduleModal` — a scrollable table (reuses `.sub-table` CSS) listing every month's installment/principal/interest/balance from `debt.schedule` (the full un-sampled array, already computed for the chart) down to a zero balance
- Verified: `npm run build` + `eslint` clean (no new warnings beyond the codebase's pre-existing baseline); amortization math cross-checked against a known-reference EMI calculator (₹100,000 principal, 12% APR, 24mo → ₹4,707.35/mo, confirmed) plus edge cases (0% interest, payment-too-low stall detection, extra payment always reduces months/interest)
