...
**Status** (updated 2026-09-24): Epic A and all of Epic C (C1 Long-Term Forecast, C2 Debt Payoff Planner, C3 Smart Rules) are fully built and shipped. Only Epic B (Household & Multi-User) remains — fully scoped, not started. Epic B's schema/RLS design has been pressure-tested by a Plan agent and spot-verified against the actual codebase (see verification notes at the top of Epic B). Ready for implementation whenever the user picks it up.
